        ...
        <section id="contact" className="py-24 relative overflow-hidden" style={{ background: 'linear-gradient(180deg,#050b18 0%,#0d0a1a 100%)' }}>
            ...
            <div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center mb-14">
                    <span className="inline-block px-4 py-1.5 rounded-full text-xs font-bold tracking-widest uppercase text-pink-400 glass border border-pink-500/20 mb-4">{t.contact.badge}</span>
                    <h2 className="text-4xl sm:text-5xl font-black gradient-text mb-4">{t.contact.title}</h2>
                ...
                <div className="glass rounded-3xl p-8 border border-white/5 mb-6" style={{ background: 'linear-gradient(135deg, rgba(99,102,241,0.08) 0%, rgba(168,85,247,0.08) 100%)' }}>
                    <div className="flex flex-col sm:flex-row items-center gap-6">
                        ...
                        <div className="text-center sm:text-left flex-1">
                            ...
                            <p className="gradient-text-blue font-medium">{t.hero.title}</p>
                ...
                <div className="grid sm:grid-cols-2 gap-4 mb-6">
                    {[
                        { href: `mailto:${profile.email}`, icon: '✉️', label: t.contact.emailLabel, value: profile.email, color: 'hover:border-indigo-500/40' },
                        { href: `tel:${profile.phone}`, icon: '📞', label: t.contact.phoneLabel, value: profile.phone, color: 'hover:border-blue-500/40' },
                    ].map((item) => (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {[
                        { href: profile.github, label: 'GitHub', emoji: '🐙', sub: t.contact.github, color: 'hover:border-white/30 hover:text-white' },
                        { href: profile.gitlab, label: 'GitLab', emoji: '🦊', sub: t.contact.gitlab, color: 'hover:border-orange-500/40 hover:text-orange-400' },
                        { href: profile.linkedin, label: 'LinkedIn', emoji: '💼', sub: t.contact.linkedin, color: 'hover:border-blue-500/40 hover:text-blue-400' },
                        { href: profile.malt, label: 'Malt', emoji: '🏆', sub: t.contact.malt, color: 'hover:border-amber-500/40 hover:text-amber-400' },
                    ].map((s) => (
